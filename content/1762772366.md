...
image: https://cdn.jsdelivr.net/gh/homarr-labs/dashboard-icons/svg/kubernetes-dashboard.svg
...
To make kubernetes do something when a CRD item is created a [controller](#controllers-and-control-loops) needs to listen for the creation event and do something to fulfill the creation and management of that item

```mermaid
flowchart TD
A@{shape: proc,label: controller}
B@{shape: doc,label: CRD records}
A -- listen to events on --> B
```
> [!NOTE] CRDs are heavily used in [crossplane](/1763642327.md)

## command tips

- get a deployment descriptor from a defined resource
