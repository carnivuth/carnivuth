...
show_image: false
draft: true
description: Come combinare operazioni per ottenere la riservatezza dei dati
...
Modalità standard di implementazione dei [cifrari a blocchi](pages/sicurezza_informazione/cifrari_simmetrici.md#cifrari-a-blocchi) questa si presenta a diverse vulnerabilità tra cui:
...
Modalità di cifratura che si rifa ai [cifrari a flusso autosincronizzanti](pages/sicurezza_informazione/cifrari_simmetrici.md#cifrari-a-flusso) con l'utilizzo di un vettore $IV$  molto adatta alla comunicazione di flussi di dati.
...
Variante del [CFB](#cipher-feedback-(cfb))  in cui pero la retroazione non prevede il cifrato al passo precedente ma il flusso di chiave stesso, adatta alla trasmissione in canali rumorosi

## Counter (ctr)

Simile a [ECB](#electronic-code-book-(ecb)), introduce il concetto di aleatorietà per mezzo di un flusso di chiave incrementale generato da un contatore cifrato, inizializzato con un seme
