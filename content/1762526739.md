...
L'azione di sort dei dati non e un operatore dell'algebra ma risulta molto utile per risolvere alcune situazioni tra cui query `ORDER BY`, [bulk loading](/b+tree#bulk-loading) degli indici e operatori di `JOIN` e `GROUP BY`.
...
## Sorting con [b+tree](/b+tree)

Nel caso di ordinamento con [b+tree](/b+tree) e necessario distinguere tra le tipologie di indice
