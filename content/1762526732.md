...
layout: simple
aliases:
  - /databases_tecnologies/progetto_fisico_tuning/index.md
  - /databases_tecnologies/progetto_fisico_tuning
permalink: /databases_tecnologies/progetto_fisico_tuning/index.md
...
In questo caso sono necessari un indice su `D.name` e un indice su `E.dno` (*possibilmente [hash](/databases_tecnologies/indici_hash)*)
...
In questo caso se si possiede un indice su `E.age` potrebbe non essere necessario l'indice per fare [index nested loop join](/databases_tecnologies/join#sfruttando-gli-indici-index-nested-loop-join), dipende da quanto e selettivo
...
Nel primo caso un indice ([b+tree](/databases_tecnologies/b+tree)) su `age,sal` può risultare conveniente, nel secondo caso l'ordine degli attributi nella query e più rilevante, in particolare:
...
Come mostrato in precedenza DB2 contempla la presenza [di indici non perfettamente clustered](/databases_tecnologies/operatori_modifica#update-della-clustering-key), di conseguenza e necessario che attributi scegliere per un indice clustered e determinare il livello di clustering desiderato
