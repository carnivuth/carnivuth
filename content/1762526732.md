...
In questo caso se si possiede un indice su `E.age` potrebbe non essere necessario l'indice per fare [index nested loop join](/join#sfruttando-gli-indici-index-nested-loop-join), dipende da quanto e selettivo
...
Nel primo caso un indice ([b+tree](/b+tree)) su `age,sal` può risultare conveniente, nel secondo caso l'ordine degli attributi nella query e più rilevante, in particolare:
