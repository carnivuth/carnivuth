...
Cifrari che si basano su algoritmi simmetrici (*e.g. [chiavi simmetriche](pages/sicurezza_informazione/chiavi.md#chiavi-simmetriche)*). Nella costruzione dei cifrari simmetrici si seguono due principi fondamentali:
...
Un cifrario a flusso richiede l'utilizzo di [PRNG sicuri](pages/sicurezza_informazione/random_number_generator.md#cryptographically-secure-pseudo-random-bit-generator-(csprng)) e la perfetta sincronia tra le due parti nell'effettuare la decifratura, inoltre essendo che si generano tanti bit di chiave quanti sono i bit del messaggio e necessario che il PRNG sia in grado di supportare un alto flusso di chiavi.
