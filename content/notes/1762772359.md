...
- filter bash history of a given day (*useful when making operations in production environments*)

```bash
sudo cat /root/.bash_history | awk '/^#/{timestamp=strftime("%F %T", substr($0,2)); next} {print timestamp, $0}'
```
