...
tags:
  ...
  - shell
  - Ci/Cd
thumbnail: https://cdn.jsdelivr.net/gh/homarr-labs/dashboard-icons/svg/git.svg
title: git
---
...
When working in agile methodology and with branches related to multiple issues it can appen that some changes needs changes from another branch that it's not been merged yet, in this case git cherry-pick can be used to apply the changes of a commit to another branch without merging the whole branch, this is useful when some changes are needed but the rest of the branch is not ready yet.

```bash
git switch branch-with-yummy-commit
git log --oneline # get hash of yummy commit
